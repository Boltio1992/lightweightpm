...
export default function ProjectDetailPage() {
  ...
  const assignable: UserPublic[] = useMemo(() => members.map((m) => m.user).filter(Boolean) as UserPublic[], [members]);
  const stats = useMemo(() => {
  ...
  function openNew() { setEditing(null); setParentFor(null); setModalOpen(true); }
  function openEdit(t: Task) { setEditing(t); setParentFor(null); setModalOpen(true); }
  function openSub(t: Task) { setEditing(null); setParentFor(t.id); setModalOpen(true); }
    ...
    <>
      <div className="border-b border-line px-8 pt-5">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="flex h-9 w-9 items-center justify-center rounded-md text-lg" style={{ backgroundColor: `${accent}20`, color: accent }}>
                {icon === "folder" && "📁"}{icon === "rocket" && "🚀"}{icon === "chart" && "📊"}{icon === "sparkles" && "✨"}{icon === "briefcase" && "💼"}{icon === "calendar" && "📅"}
              </span>
            ...
            <p className="mt-1 text-xs text-muted">{fmtDate(project.start_date)} → {fmtDate(project.end_date)} · {stats.done}/{stats.total} tasks done ({stats.pct}%){stats.overdue > 0 && <span className="text-danger"> · {stats.overdue} overdue</span>}</p>
          </div>
          <div className="flex flex-none items-center gap-2">
            <select className="input w-auto" value={project.status} onChange={(e) => changeStatus(e.target.value)}>
              <option value="active">Active</option><option value="on_hold">On Hold</option><option value="done">Done</option><option value="archived">Archived</option>
            </select>
        ...
        <div className="mt-4 flex gap-1">
          {TABS.map((t) => <button key={t.key} onClick={() => setTab(t.key)} className={`-mb-px border-b-2 px-3 py-2 text-sm transition ${tab === t.key ? "border-ink font-medium text-ink" : "border-transparent text-muted hover:text-ink"}`}>{t.label}{t.key === "members" && <span className="ml-1.5 text-xs text-muted">{members.length}</span>}</button>)}
        </div>
      </div>
      ...
      <TaskModal open={modalOpen} onClose={() => setModalOpen(false)} onSaved={load} task={editing} projectId={projectId} parentTaskId={parentFor} assignableUsers={assignable} />
      <ProjectSettingsModal open={settingsOpen} project={project} onClose={() => setSettingsOpen(false)} onSaved={load} />
    </>
  );
}
